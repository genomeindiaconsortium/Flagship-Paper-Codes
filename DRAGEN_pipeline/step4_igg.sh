...
#SBATCH --job-name=dragen_igg_demo_V1Step4
#SBATCH --output=V1Step4/%A_%a_V1Step4.out
#SBATCH --error=V1Step4/%A_%a_V1Step4.err
...
for job in $(make_jobs | awk 'NR>='$istart' && NR<='$iend''); do
    ...
    echo </path/to/your/>run-igg.v4.2.sh step4 $version $merge $shard
    </path/to/your/>run-igg.v4.2.sh step4 $version $merge $shard
done
